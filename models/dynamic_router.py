...
# 动态账户路由 — 基于监控数据自动安排不同时段优先使用不同 API 账号

# 数据来源:
#   - maintenance 的 account_check 任务（周期性探测，source="check"）
#   - FailoverChat 每次真实请求的成功/失败/延迟（source="request"）
# 学习结果:
#   为每个受管账号生成 time_slots（source="dynamic"），
#   使同一时段内可用性高的账号优先级更低（更优先被尝试）。
# 手动时段（source="manual"，由 /login timeslot 配置）永远压过动态学习时段。
...
class MonitorStore:
    """监控观察存储 — 线程安全内存缓存 + JSON 持久化 (.dsn/api_monitor.json)"""

    def __init__(self, path: str | None = None):
        self._path = path or _MONITOR_FILE
...
class DynamicRouter:
    ...
    def __init__(self, store: MonitorStore | None = None, api_manager=None):
        self._store = store or MonitorStore()
        self._mgr = api_manager
        self._window = DEFAULT_WINDOW
        self._min_obs = DEFAULT_MIN_OBS
    ...
    def _manager(self):
        if self._mgr is None:
            from .api_accounts import get_api_manager
            self._mgr = get_api_manager()
        return self._mgr
    ...
    def plan(self) -> dict[int, list[tuple[str, int]]]:
        """每个小时各账号的动态排序。返回 {hour: [(account, rank), ...]}"""
        mgr = self._manager()
    ...
    def recompute(self) -> dict:
        """基于监控数据为每个受管账号重算动态时段（source=dynamic）。
        ...
        self._store.flush()
        logger.info("动态路由重算完成: %d 个账号写入动态时段", applied)
    ...
    @staticmethod
    def _manual_hours(acc) -> set[int]:
        """返回被手动时段覆盖的小时集合（手动 time_slots + /login schedule）。"""
        covered: set[int] = set()
        ...
        # 全局手动时段安排（凌驾于优先级系统，动态学习绝不能覆盖）
        try:
            from .api_accounts import _current_manual_schedule
            for rule in _current_manual_schedule():
