...
- [x] 首次启动引导系统：main.py 自动检测未配置状态、调用 onboarding.py 交互式配置向导
- [x] onboarding.py AI 引导流程：角色卡配置 + 功能组逐项配置，[CONFIG_COMPLETE] 标记精确完成
- [x] 配置向导网络容错：自动重试 + ^C 任意时刻安全退出 + 终端清屏
