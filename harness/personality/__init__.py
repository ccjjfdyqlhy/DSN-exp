...
from .traits import (
    ...
    default_indicator_vector, random_indicator_vector, clamp_vector, deviant_dimensions, format_deviant_dimensions,
)

logger = logging.getLogger("PersonalitySystemV3")


def _find_default_cards_dir() -> Path:
    """自动查找角色卡目录。
    1. 项目根目录/apps/dsn/character_cards
    2. 项目根目录/character_cards
    3. 本文件同级/上级的 character_cards
    """
    repo_root = Path(__file__).resolve().parent.parent.parent
    candidate1 = repo_root / "apps" / "dsn" / "character_cards"
    if candidate1.exists():
        return candidate1
    candidate2 = repo_root / "character_cards"
    if candidate2.exists():
        return candidate2
    candidate3 = Path(__file__).resolve().parent / "character_cards"
...
class PersonalitySystemV3:
    def __init__(
        ...
        self.db = db
        self._cards_dir = Path(cards_dir) if cards_dir else _find_default_cards_dir()
        self._cards_dir.mkdir(parents=True, exist_ok=True)
        self._default_card_path = default_card_path or str(self._cards_dir / "exa.yaml")
        logger.info("V3: 角色卡目录=%s, 默认角色卡路径=%s", self._cards_dir, self._default_card_path)

        self._persistence = V3Persistence(db)
        self._evidence = EvidenceAccumulator(db)
        self._audit = AuditLogger(db)
        self._state_manager = V3StateManager(
            self._persistence,
    ...
    def create_blank_card(
        ...
        name: str = "空白角色",
        seed: int | None = None,
        randomize_traits: bool = True,
        auto_save: bool = True,
    ) -> CharacterCard:
        """创建一张从空白演化的角色卡，可带有随机性格参数，支持后续对话自行演化。"""
        card = CharacterCard.create_blank(
            card_id=card_id,
            name=name,
            seed=seed,
    ...
    def load_default_card(self) -> CharacterCard | None:
        ...
        try:
            ...
            logger.warning("V3: 默认角色卡文件不存在: %s，自动生成空白演化角色卡", self._default_card_path)
            card_id = Path(self._default_card_path).stem or "blank"
            card = self.create_blank_card(card_id=card_id, name="初始伴侣", auto_save=True)
            return card
    ...
    def _backup_card(self, card) -> None:
        ...
        from datetime import datetime as _dt, timezone as _tz
        backup_dir = self._cards_dir / "backups" / card.card_id
    ...
    def list_backups(self, card_id: str) -> list[dict]:
        ...
        backup_dir = self._cards_dir / "backups" / card_id
    ...
    def _maybe_consolidate(self, card_id: str) -> None:
        if not self._evidence.should_consolidate(card_id):
            return
        distilled_json_path = self._cards_dir / f"{card_id}.distilled.json"
        card = self.get_card(card_id)
        merged = self._evidence.consolidate(card_id, distilled_json_path, card=card)
        if merged:
