...
from .traits import TRAIT_IDS, default_indicator_vector, random_indicator_vector
...
class CharacterCard:
    ...
    def create_blank(
        ...
        card_id: str = "blank",
        name: str = "空白角色",
        display_name: str = "空白角色",
        seed: int | None = None,
        randomize_traits: bool = True,
        description: str = "从白纸开始自行随对话交互演化的人格",
    ) -> CharacterCard:
        """创建一张空白角色卡。
        可携带随机初始化的 50 维性格参数，随用户对话从零演化。
        """
        import random as _rand
        actual_seed = seed if seed is not None else _rand.randint(1000, 999999)
        overrides = {}
        if randomize_traits:
            overrides = random_indicator_vector(seed=actual_seed)
        ...
        return cls(
            ...
            natural_language=NaturalLanguage(
                personality="一张尚未定型的白纸，随着与用户的交互和相处逐渐发展出独一无二的性格与偏好。",
                behavior="保持开放与好奇，对未知保持敏感。",
                speech_style="自然、真诚，随交流逐步形成自己的表达习惯。",
            ),
            dynamic_config=DynamicConfig(
                seed=actual_seed,
                noise_amplitude=0.15,
                mood_volatility=0.20,
                temporal_drift_rate=0.03,
                response_inertia=0.30,
            ),
