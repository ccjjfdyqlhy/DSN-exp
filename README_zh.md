...
### 🧠 记忆系统
- **话题**：对话按 30min 静默 + 语义无关自动分段为话题；当前/激活话题注入原文，关闭话题注入聚合摘要
- **自动摘要**：LLM驱动压缩，AES-256-GCM加密
- **向量检索**：768维语义搜索，支持模糊查询
- **话题激活**：词嵌入被动激活(本轮) + 工具主动激活(持续到话题结束)
- **全局备忘**：memo 为跨话题长期事实，不随话题关闭而模糊
- **记忆类型**：话题对话原文 + 每轮摘要(`exp`) + 手动备忘(`memo`)
...
```
新消息 ─▶ 词嵌入打分 + (歧义时)LLM judge ─▶ 归属话题
        ├─ 续当前 → 话题保持打开, 原文注入
        ├─ 重开旧话题 → 打开该话题(被动1轮 / 主动持续)
        └─ 新话题 → 关闭旧话题, 新建话题
对话原文 → 按话题存 messages.topic_id
每轮摘要 → LMSummaryModel → AES-256加密 → memory_v2(topic_id)
关闭话题 → 聚合摘要 + 标题 + 话题向量
激活话题 → 原文块注入上下文; 关闭话题 → 仅聚合摘要
```

**特性：**
- 话题分段：30min 静默 + 语义无关自动划新话题
- 话题激活：被动激活(词嵌入命中, 本轮) / 主动激活(`activate_topic` 工具, 持续到话题结束)
- 上下文组装：当前/激活话题原文清晰，闭锁话题仅见聚合摘要，memo 全局常驻
- 向量检索：支持语义模糊搜索
