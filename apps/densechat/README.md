# apps/densechat — DSN-exp Harness 复刻的 DenseChat WebUI
...
```bash
# 从项目根目录启动（默认工作区为当前目录）
python main.py --app densechat

# 指定项目目录/端口（launcher 透传参数需用 -- 分隔）
python main.py --app densechat -- --project /path/to/project --port 8080

# 或直接运行
python -m apps.densechat --project /path/to/project --port 8080
```
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `DENSECHAT_API_KEY` / `OPENAI_API_KEY` | - | OpenAI 兼容 API Key |
| `DENSECHAT_BASE_URL` / `OPENAI_API_BASE` | `https://api.deepseek.com/v1` | API 地址 |
| `DENSECHAT_FLASH_MODEL` / `FLASH_MODEL` | `deepseek-v4-flash` | Flash 模型 |
| `DENSECHAT_PRO_MODEL` / `PRO_MODEL` | `deepseek-v4-pro` | Pro 模型 |
| `DENSECHAT_OPENAI_MODEL` / `OPENAI_MODEL` | `gpt-4o-mini` | OpenAI 模型 |
| `DENSECHAT_PROJECT` | 当前目录 | 工作区项目路径 |
| `DENSECHAT_DB` | `<project>/.densechat_web.db` | 会话数据库 |
| `DENSECHAT_PORT` | `8080` | Web 端口 |
...
- 技能加载：自动扫描 `apps/densechat/skills`（或 `DENSECHAT_SKILLS_DIR`），支持 `TOOLS` / `register` / `SKILL` 约定，可在 WebUI 热重载。
...
- 后端：不依赖 densechat 私有模块，改用 harness 基座：
  ...
  - FastAPI/uvicorn 提供与 DenseChat 一致的 `/ws` 与 `/api/*` 协议
