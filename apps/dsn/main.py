...
if not _is_env_configured():
    print("\n  ⚠ 检测到未配置状态，进入引导流程...\n")
    try:
        from apps.dsn.onboarding import run as run_onboarding
...
def _login_list(mgr):
    ...
    if not accounts:
        ...
        return
    from apps.dsn.models.api_accounts import is_known_down, _known_down_window
...
def _cmd_detail(arg: str = ""):
    ...
    from apps.dsn.models import DETAIL_CHATS, DETAIL_ACTIONS, toggle_detail_chats, toggle_detail_actions
...
def main():
    ...
    if getattr(Config, "DEBUG_PLAY_AS_MODEL", False):
        try:
            from apps.dsn.api.debug import create_debug_app
    ...
    if getattr(Config, "WEB_ADMIN_ENABLED", True):
        try:
            ...
            from apps.dsn.web_admin.server import start_admin_server
    ...
    if getattr(Config, "STEWARD_ENABLED", True):
        try:
            from apps.dsn.utils.steward import StewardModel
