...
def _get_stream_service():
    try:
        from apps.dsn.api.vision import stream_service as _ss
        if _ss is not None:
            return _ss
    except Exception:
        pass
    try:
        from apps.dsn.api.vision import coordinator
...
def _import_main_components():
    from apps.dsn.api import app as app_module
...
def api_status():
    from apps.dsn.main import _server_start_time
...
def api_pairing_generate():
    ...
    sys.stdout = buf
    try:
        from apps.dsn.main import _cmd_newbind
...
def api_memory_list(uid, cid):
    ...
    from apps.dsn.utils.crypto import MessageCipher
    cipher = db._cipher
    if round_str:
        try:
...
def api_config_list():
    Config = C()
    from apps.dsn.main import SENSITIVE_CONFIG_KEYS, READONLY_CONFIG_KEYS, _mask_value, _env_backup_count
    items = []
...
def api_config_set():
    ...
    from apps.dsn.main import READONLY_CONFIG_KEYS, SENSITIVE_CONFIG_KEYS
    ...
    from apps.dsn.main import _env_backup_rotate, _env_write, append_log
    _env_backup_rotate()
...
def api_config_undo():
    Config = C()
    from apps.dsn.main import _env_backup_count, _env_backup_restore
...
def api_persona_status(card_id):
    ...
    try:
        from apps.dsn.prompt.personality_v3.traits import TRAIT_MAP
...
def api_logs():
    from apps.dsn.main import get_logs_snapshot
...
def api_reminder_skip():
    ...
    if not tm:
        return jsonify({"error": "TaskManager unavailable"}), 503
    from apps.dsn.tasks import TaskStatus, TaskType
    if tid in tm.tasks:
        task = tm.tasks[tid]
...
def api_hibernate_check():
    ...
    from apps.dsn.maintenance import config as mc
...
def api_timer_toggle():
    from apps.dsn.plugins.pipeline import toggle_timer
    enabled = toggle_timer()
    return jsonify({"success": True, "enabled": enabled})

@admin_bp.route("/stop", methods=["POST"])
def api_stop():
    from apps.dsn.main import append_log
