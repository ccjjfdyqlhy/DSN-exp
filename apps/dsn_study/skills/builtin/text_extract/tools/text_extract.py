...
class TextExtractTool:
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        if models is not None and hasattr(models, "send_message"):
            return str(models.send_message(prompt))
        if models is not None and hasattr(models, "invoke"):
            try:
                import importlib
                ctx_mod = importlib.import_module("plugins.base")
                ctx = getattr(ctx_mod, "PluginContext")()
                return str(models.invoke(
                ...
                pass
        try:
            from harness.models import OpenAICompatClient, ChatMessage
            import os
            ...
            client = OpenAICompatClient(api_key=api_key, base_url=api_base, model=model)
            resp = client.invoke([ChatMessage.user(prompt)])
            return resp.content or ""
        except Exception:
            try:
                import importlib
                clients_mod = importlib.import_module("apps.dsn.models.clients")
                chat_cls = getattr(clients_mod, "OpenAIChat")
                chat = chat_cls()
                return str(chat.send_message(prompt))
            except Exception:
                return ""
